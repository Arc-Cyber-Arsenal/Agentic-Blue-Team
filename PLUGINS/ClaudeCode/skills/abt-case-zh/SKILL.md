---
name: ABT-case-zh
description: '管理 ABT 安全 case。适用于审查 case、列出 case、查看 case 讨论、检查 case 相关告警，或更新 case 工作流和 AI 分析字段,附加外部 ticket 到 case。'
...
Case 是 ABT 中的核心调查对象，一个 Case 会挂载一个或多个 Alert, 一个 Alert 挂载一个或多个 Artifact,所以用户通常调查过程只操作
Case。
...
6. 如果用户给出了多个过滤条件，只应用 ABT 直接支持的部分，并明确说明不支持的过滤条件。
7. 如果用户要把 enrichment 或结构化分析附加到 case，使用 `ABT-enrichment-zh` skill。
8. 如果用户要把外部 ticket 附加到 case，使用 `ABT-ticket-zh` skill。
