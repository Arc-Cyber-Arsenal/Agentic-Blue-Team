---
name: ABT-siem-en
description: 'Investigate ABT SIEM data with schema exploration, keyword search, and adaptive queries. Use when users want to find the right index, inspect available fields, search logs by IOC, or run structured hunts with exact filters and aggregations.'
...
Use this skill when the user needs SIEM investigation on ABT. The focus is on choosing the right query path and returning evidence that is useful for analysis.
