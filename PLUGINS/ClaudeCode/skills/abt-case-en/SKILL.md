---
name: ABT-case-en
description: 'Manage ABT security cases, review discussions, update workflow or AI analysis fields, attach enrichment, or attach external tickets.'
argument-hint: 'review case <case_id> | list cases [filters] | update case <case_id> <fields>'
compatibility: connect to ABT mcp server
metadata:
  author: Funnywolf
  version: 0.3.0
  mcp-server: ABT
  ...
  documentation: https://ABT.viperrtp.com/
---

# ABT Case

Use this skill when the user wants to work on ABT cases in a case-centric SOC flow.
Case is the core investigation object in ABT. One case can have one or more alerts, and one alert can have one or more artifacts, so users usually work primarily at the case level.
...
6. If the user gives multiple filters, apply only the ones ABT supports directly and state any unsupported filters explicitly.
7. If the user wants to attach enrichment or structured analysis to the case, use the `ABT-enrichment-en` skill.
8. If the user wants to attach an external ticket to the case, use the `ABT-ticket-en` skill.
...
- Ask for enum clarification only when the requested value does not map cleanly to ABT values.
