---
name: ABT-alert-zh
description: '审查 ABT 告警、更新 AI 分析字段、查看告警讨论。'
...
当用户要围绕 ABT 告警开展 SOC 分析工作时，使用这个 skill。
Alert 是 ABT 中的二级数据,每个 Alert 都会挂载到一个 Case,一个 Alert 会挂载一个或多个 Artifact。
...
5. 如果用户要附加分析结果、情报或结构化上下文，使用 `ABT-enrichment-zh` skill。
...
- 只有当请求值不能清晰映射到 ABT 枚举时，才要求用户澄清枚举值。
