---
name: ABT-knowledge-zh
description: 'ABT 平台内部 Knowledge 记录的检索与维护。支持在数据库中筛选 knowledge 记录，也支持通过 search_knowledge 在向量数据库中做关键词或语义搜索。'
...
当用户要在 ABT 中检索或维护内部知识库时，使用这个 skill。

# 设计思路

ABT 的内部 Knowledge 本质上是一条一条数据库记录，核心字段包括 `title`、`body`、`using`、`action`、`source`、`tags`。
